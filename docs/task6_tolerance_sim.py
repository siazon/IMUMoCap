"""
Task 6: Tolerance lower-bound simulation on historical FPA data.

Pipeline: identical to task5_fpa.py (calibration → stance → context → PD → FPA).
After collecting all FPA steps:
  - First BASELINE_N steps (per foot) → compute μ, SD, target T = μ ± 5°
  - Remaining steps → simulate hit rate for different tolerance formulas

Tolerance formulas compared:
  Current:  clamp(SD, 6, 10)          half-width directly
  Alt:      max(w_min, SD)            for w_min ∈ {2, 3, 4, 5} degrees
"""
import sys, math, csv
from collections import deque

sys.stdout.reconfigure(encoding='utf-8')
CSV_PATH = sys.argv[1] if len(sys.argv) > 1 else r"D:\SourceCode\IMUMoCap\docs\ImuSamples_20260609_225004.csv"
BASELINE_N = 20   # steps per foot used as baseline
...
# ── main loop: collect all FPA steps ─────────────────────────────────────────
all_fpa = []   # list of (fpa_l, fpa_r), NaN if foot not emitted this step
...
print(f"Total FPA steps collected: {len(all_fpa)}")
...
# separate left / right indices to handle NaN correctly
l_steps = [fpa_l for (fpa_l,_) in all_fpa if not math.isnan(fpa_l)]
r_steps = [fpa_r for (_,fpa_r) in all_fpa if not math.isnan(fpa_r)]

print(f"\nLeft  valid steps: {len(l_steps)},  Right valid steps: {len(r_steps)}")

if len(l_steps)<BASELINE_N+1 or len(r_steps)<BASELINE_N+1:
    print(f"ERROR: not enough steps for baseline ({BASELINE_N}) + at least 1 training step.")
    sys.exit(1)

bl_L = l_steps[:BASELINE_N];  train_L = l_steps[BASELINE_N:]
bl_R = r_steps[:BASELINE_N];  train_R = r_steps[BASELINE_N:]

mu_L, sd_L = stats(bl_L)
mu_R, sd_R = stats(bl_R)

# target (current: μ ± 5°)
def compute_target(mu):
    if mu > 10.0: return mu - 5.0, 'ToeIn'
    else:         return mu + 5.0, 'ToeOut'

t_L, dir_L = compute_target(mu_L)
t_R, dir_R = compute_target(mu_R)

print(f"\n── Baseline stats (first {BASELINE_N} steps per foot) ──")
print(f"  Left:   μ={mu_L:+.2f}°  SD={sd_L:.2f}°  → target={t_L:+.2f}° ({dir_L})")
print(f"  Right:  μ={mu_R:+.2f}°  SD={sd_R:.2f}°  → target={t_R:+.2f}° ({dir_R})")
print(f"  Training steps: L={len(train_L)}  R={len(train_R)}")

# ── tolerance simulation ───────────────────────────────────────────────────────
def hit_rate(steps, target, tol_half):
    if not steps: return float('nan')
    hits = sum(1 for v in steps if abs(v-target) <= tol_half)
    return 100.0*hits/len(steps)

def clamp(v, lo, hi): return max(lo, min(hi, v))

print(f"\n── Simulation: hit rate vs tolerance lower bound ──")
print(f"  (Training set: L={len(train_L)} steps, R={len(train_R)} steps)")
print()

# header
print(f"  {'Formula':<26}  {'tol_L':>6}  {'hit_L':>7}  {'tol_R':>6}  {'hit_R':>7}  {'avg_hit':>8}")
print("  " + "-"*70)

# current formula: clamp(SD, 6, 10)
for lo, hi in [(6,10)]:
    tol_l = clamp(sd_L, lo, hi)
    tol_r = clamp(sd_R, lo, hi)
    hl = hit_rate(train_L, t_L, tol_l)
    hr = hit_rate(train_R, t_R, tol_r)
    tag = f"clamp(SD,{lo},{hi})  [current]"
    print(f"  {tag:<26}  {tol_l:>6.2f}°  {hl:>6.1f}%  {tol_r:>6.2f}°  {hr:>6.1f}%  {(hl+hr)/2:>7.1f}%")

print()

# alternative: max(w_min, SD)  for different w_min
for w_min in [2, 3, 4, 5, 6]:
    tol_l = max(w_min, sd_L)
    tol_r = max(w_min, sd_R)
    hl = hit_rate(train_L, t_L, tol_l)
    hr = hit_rate(train_R, t_R, tol_r)
    tag = f"max({w_min}°, SD)"
    marker = "  ← current SD floor" if w_min==6 else ""
    print(f"  {tag:<26}  {tol_l:>6.2f}°  {hl:>6.1f}%  {tol_r:>6.2f}°  {hr:>6.1f}%  {(hl+hr)/2:>7.1f}%{marker}")

print()

# show error distribution for intuition
def pct(steps, target, tol):
    if not steps: return []
    errs = sorted(abs(v-target) for v in steps)
    result = []
    for t in tol:
        c = sum(1 for e in errs if e<=t)
        result.append(100.0*c/len(errs))
    return result

print(f"── Error CDF (|FPA - target|) ──")
print(f"  Left  (n={len(train_L)}):  target={t_L:+.1f}°")
thresholds = [1, 2, 3, 4, 5, 6, 7, 8, 10]
pct_L = pct(train_L, t_L, thresholds)
pct_R = pct(train_R, t_R, thresholds)
print(f"  {'|err|≤':>8}", end="")
for t in thresholds: print(f"  {t}°", end="")
print()
print(f"  {'Left':>8}", end="")
for p in pct_L: print(f"  {p:3.0f}%", end="")
print()
print(f"  Right (n={len(train_R)}):  target={t_R:+.1f}°")
print(f"  {'Right':>8}", end="")
for p in pct_R: print(f"  {p:3.0f}%", end="")
print()

# raw error spread
print(f"\n── Error spread (|FPA - target|, training steps) ──")
if train_L:
    errs_L=sorted(abs(v-t_L) for v in train_L)
    print(f"  Left:  min={errs_L[0]:.1f}°  p25={errs_L[len(errs_L)//4]:.1f}°  median={errs_L[len(errs_L)//2]:.1f}°  p75={errs_L[3*len(errs_L)//4]:.1f}°  max={errs_L[-1]:.1f}°")
if train_R:
    errs_R=sorted(abs(v-t_R) for v in train_R)
    print(f"  Right: min={errs_R[0]:.1f}°  p25={errs_R[len(errs_R)//4]:.1f}°  median={errs_R[len(errs_R)//2]:.1f}°  p75={errs_R[3*len(errs_R)//4]:.1f}°  max={errs_R[-1]:.1f}°")
