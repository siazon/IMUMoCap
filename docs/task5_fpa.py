"""
Task 5: FPA output verification
Mirrors FpaEngine.cs (StanceSampler, gate, FPA = foot_yaw - PD, deg conversion)
"""
import sys, math, csv
...
def extract_yaw_z(q):   # FpaEngine.ExtractYaw — always Z axis
...
def process_fpa(lq,rq,ls,rs,ctx_now,conf,pd_dir,pd_valid,pd_stab):
    ...
    if not ctx_ok or not pd_ok: return None, 'gate'
    if not ls and not rs: return None, 'double-swing'
    # Step 3: settle
    fpa_l_rad=l_sampler.try_settle(MIN_SETTLE)
    fpa_r_rad=r_sampler.try_settle(MIN_SETTLE)
    if fpa_l_rad is None and fpa_r_rad is None: return None, 'settling'
    ...
    return (fpa_l, fpa_r), None

# ── main loop ─────────────────────────────────────────────────────────────────
print(f"{'pid':>7} {'ctx':>14} {'conf':>5} {'pdV':>4} {'pdS':>5}  {'fpaL°':>7} {'fpaR°':>7}  note")
print("-"*75)

fpa_results=[]; null_reasons={'gate':0,'double-swing':0,'settling':0}
...
for pid in pids:
    ...
    result,reason=process_fpa(f['Left']['q'],f['Right']['q'],ls,rs,ctx_now,conf,pd_dir,pd_is_valid,pd_stab)
    if reason: null_reasons[reason]+=1

    if result is not None:
        fpa_l,fpa_r=result
        fpa_results.append((pid,fpa_l,fpa_r,ctx_now,math.degrees(pd_dir)))
        print(f"{pid:>7} {ctx_now:>14} {conf:>5.2f} {'Y':>4} {pd_stab:>5.3f}  {fpa_l:>+7.2f} {fpa_r:>+7.2f}")

# ── summary ───────────────────────────────────────────────────────────────────
print(f"\nTotal FPA outputs: {len(fpa_results)}")
if fpa_results:
    ls_vals=[r[1] for r in fpa_results if not math.isnan(r[1])]
    rs_vals=[r[2] for r in fpa_results if not math.isnan(r[2])]
    if ls_vals: print(f"Left  FPA: min={min(ls_vals):+.1f}°  max={max(ls_vals):+.1f}°  mean={sum(ls_vals)/len(ls_vals):+.1f}°  n={len(ls_vals)}")
    if rs_vals: print(f"Right FPA: min={min(rs_vals):+.1f}°  max={max(rs_vals):+.1f}°  mean={sum(rs_vals)/len(rs_vals):+.1f}°  n={len(rs_vals)}")
    in_range_l=sum(1 for v in ls_vals if -15<=v<=20)
    in_range_r=sum(1 for v in rs_vals if -15<=v<=20)
    print(f"In range [-15°,+20°]: L={in_range_l}/{len(ls_vals)}  R={in_range_r}/{len(rs_vals)}")
print(f"\nNull frames by reason: {null_reasons}")
